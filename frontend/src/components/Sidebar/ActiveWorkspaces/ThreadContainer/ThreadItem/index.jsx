...
function OptionsMenu({ containerRef, workspace, thread, onRemove, close }) {
  ...
  const renameThread = async () => {
    const name = window.prompt(t("threadItem.threadRenamePrompt"))?.trim();
    ...
    if (!!message) {
      showToast(t("threadItem.threadUpdateError", { message }), "error", {
        clear: true,
      });
      close();
      return;
  ...
  const handleDelete = async () => {
    if (!window.confirm(t("threadItem.confirmDelete"))) return;
    const success = await Workspace.threads.delete(workspace.slug, thread.slug);
    if (!success) {
      showToast(t("threadItem.threadDeleteError"), "error", { clear: true });
      return;
    }
    if (success) {
      showToast(t("threadItem.threadDeleteSuccess"), "success", {
        clear: true,
      });
      onRemove(thread.id);
      return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-sm">{t("threadItem.renameThread")}</p>
      </button>
      <button
