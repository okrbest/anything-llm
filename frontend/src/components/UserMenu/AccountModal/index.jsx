...
export default function AccountModal({ user, hideModal }) {
  ...
  const handleFileUpload = async (event) => {
    ...
    if (!success) {
      showToast(t("accountModal.uploadError", { error }), "error");
      return;
    }

    const pfpUrl = await System.fetchPfp(user.id);
    setPfp(pfpUrl);
    showToast(t("accountModal.uploadSuccess"), "success");
  };

  const handleRemovePfp = async () => {
    const { success, error } = await System.removePfp();
    if (!success) {
      showToast(t("accountModal.removeError", { error }), "error");
      return;
    }
  ...
  const handleUpdate = async (e) => {
    ...
    if (success) {
      ...
      showToast(t("accountModal.updateSuccess"), "success", { clear: true });
      hideModal();
    } else {
      showToast(t("accountModal.updateError", { error }), "error");
    }
  };
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-8">
            <div className="flex flex-col items-center">
              ...
              {pfp && (
                ...
                >
                  {t("accountModal.removeProfilePicture")}
                </button>
              )}
            </div>
          </div>
  ...
  return (
    <div>
      <label
        ...
        className="block mb-2 text-sm font-medium text-white"
      >
        {t("accountModal.preferredLanguage")}
      </label>
      <select
        name="userLang"
